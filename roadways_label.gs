function generate_roadways_shipping_label(data) {
  ...
  fileName = fileName + "RoadwaysShippingLabel";
  let target_sheet = getSheet("Roadways Address Label")
  ...
  if (data) {
    set_data(required_columns, data, target_sheet) // setting the data in the "Roadways Address Label" sheet
    ...
    data = data.filter(row => row[get_column_index.get("Posting Type")].toLowerCase() === "roadways");
    data = data.map((row) => {
  ...
  data.forEach(function (row) {
    ...
    let label_data = {};
    let address_data = {};
    address_data.ss_no = row[final_columns_index.get("subscription_number")].trim()
    ...
    address_data.despatched_through = row[final_columns_index.get("Despatched through")].trim()
    address_data.destination_place = row[final_columns_index.get("Destination Place")].trim()
  ...
  let html = getTableHtml(all_labels_data, "roadways_label_template.html");
